# Step13: Figure 3B mouse Hallmark GSEA running enrichment curves.
# Seed-locked revision: rerun mouse Hallmark GSEA with fixed random seeds and nproc = 1.
# P-value handling revision: replace non-positive or non-finite P values with .Machine$double.xmin before ranking.
# Purpose:
# Generate Figure 3B using the locked mouse discovery DE results and the same targeted
# Hallmark GSEA logic used for pig early/chronic pathway interpretation.
#
# Locked method:
...
#    EMT / ECM remodeling-related transcriptional module, but the actual gene set is the
...
old_outputs <- c(list.files(table_dir, full.names = TRUE, recursive = TRUE), png_file, pdf_file, log_file)
unlink(old_outputs[file.exists(old_outputs)], recursive = TRUE, force = TRUE)

sink(log_file, split = TRUE)

cat("===== STEP13 FIGURE3B MOUSE HALLMARK GSEA =====\n")
cat("Run time: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z"), "\n", sep = "")
cat("DE directory: ", de_dir, "\n", sep = "")
...
pick_col <- function(df, candidates, required = TRUE, what = NULL) {
  ...
  if (required) {
    stop("Cannot find required column", if (!is.null(what)) paste0(" for ", what) else "", ". Checked: ", paste(candidates, collapse = ", "))
  }
  NA_character_
}

find_locked_de_file <- function(timepoint) {
  files <- list.files(de_dir, pattern = "\\.csv$", full.names = TRUE, recursive = FALSE)
  if (length(files) == 0) stop("No CSV files found in DE directory: ", de_dir)
  tp_pat <- if (timepoint == "1W") "(^|_|-)1W($|_|-|\\.)|1w" else "(^|_|-)4W($|_|-|\\.)|4w"

  candidates <- files[
    grepl(tp_pat, basename(files), ignore.case = TRUE) &
      grepl("(step06|DE|de|limma|voom|topTable)", basename(files), ignore.case = TRUE) &
      !grepl("(voom_logCPM|PCA|rank|GSEA|fgsea|summary|extract|volcano_source|Figure|plot)", basename(files), ignore.case = TRUE)
  ]

  if (length(candidates) == 0) {
    candidates <- files[
      grepl(tp_pat, basename(files), ignore.case = TRUE) &
        !grepl("(voom_logCPM|PCA|rank|GSEA|fgsea|summary|extract|Figure|plot)", basename(files), ignore.case = TRUE)
    ]
  }
  ...
  for (ff in candidates) {
    ok <- tryCatch({
      tab <- read.csv(ff, nrows = 20, stringsAsFactors = FALSE, check.names = FALSE)
      has_logfc <- any(c("logFC", "log2FC", "log2FoldChange") %in% colnames(tab))
      has_p <- any(c("P.Value", "PValue", "P.Value.", "pvalue", "P") %in% colnames(tab))
      has_symbol <- any(c("SYMBOL", "gene_symbol", "mouse_symbol", "GeneSymbol", "symbol", "Symbol") %in% colnames(tab))
      has_logfc && has_p && has_symbol
    }, error = function(e) FALSE)
    if (ok) good <- c(good, ff)
  }

  if (length(good) == 0) {
    all_csv <- data.frame(file = basename(files), path = files, size_bytes = file.info(files)$size, stringsAsFactors = FALSE)
    write_csv(all_csv, file.path(table_dir, paste0("step13_all_csv_files_seen_when_missing_DE_", timepoint, ".csv")))
    stop("Cannot identify locked Step06 DE table for ", timepoint, ". See file list saved in table_dir.")
  }

  step06 <- good[grepl("step06", basename(good), ignore.case = TRUE)]
  if (length(step06) > 0) good <- step06
  sizes <- file.info(good)$size
  good[which.max(sizes)]
}
...
make_rank_table <- function(de_file, timepoint) {
  ...
  fdr_col <- pick_col(de, c("adj.P.Val", "FDR", "padj", "adj_p", "adj.P.Val.", "qvalue", "q_value"), required = FALSE)
...
enrichment_curve_df <- function(stats, pathway_genes, pathway_name, timepoint, nes, padj, pval) {
  ...
  data.frame(
    timepoint = timepoint, pathway = pathway_name, rank = seq_along(stats),
    running_ES = running, is_hit = hits, gene_symbol = names(stats),
    NES = nes, padj = padj, pval = pval, stringsAsFactors = FALSE
  )
}
...
de_1w_file <- find_locked_de_file("1W")
de_4w_file <- find_locked_de_file("4W")
cat("Selected 1W DE file:\n", de_1w_file, "\n", sep = "")
cat("Selected 4W DE file:\n", de_4w_file, "\n\n", sep = "")
...
ranks_1w <- rank_1w$rank_stat; names(ranks_1w) <- rank_1w$gene_symbol; ranks_1w <- sort(ranks_1w, decreasing = TRUE)
ranks_4w <- rank_4w$rank_stat; names(ranks_4w) <- rank_4w$gene_symbol; ranks_4w <- sort(ranks_4w, decreasing = TRUE)

msig_raw <- msigdbr::msigdbr(species = "Mus musculus", collection = "H")
...
hallmark_source <- data.frame(pathway = rep(names(pathways), lengths(pathways)), gene_symbol = unlist(pathways, use.names = FALSE), stringsAsFactors = FALSE)
...
fgsea_1w$timepoint <- "1W"; fgsea_4w$timepoint <- "4W"
...
target_stats <- target_stats[order(target_stats$pathway, target_stats$timepoint), , drop = FALSE]
...
if (length(missing_targets) > 0) stop("Some target pathways were not found in fgsea results: ", paste(missing_targets, collapse = ", "))
...
for (tp in c("1W", "4W")) {
  ...
  for (pw in target_pathways) {
    ...
    curve_list[[paste(tp, pw, sep = "__")]] <- enrichment_curve_df(
      stats = ranks, pathway_genes = pathways[[pw]], pathway_name = pw, timepoint = tp,
      nes = row$NES[1], padj = row$padj[1], pval = row$pval[1]
    )
  }
}
...
## Keep the same 2 x 2 visual organization as the previous Figure3B:
## row 1 = Inflammatory response, row 2 = EMT / ECM remodeling;
## column 1 = 1W, column 2 = 4W.
panel_order <- c(
...
summary_df <- data.frame(
  metric = c("DE_file_1W", "DE_file_4W", "rank_method", "rank_direction", "p_value_handling", "n_ranked_genes_1W", "n_ranked_genes_4W", "n_p_values_replaced_1W", "n_p_values_replaced_4W", "Hallmark_species", "Hallmark_collection", "Hallmark_pathways_tested", "fgsea_minSize", "fgsea_maxSize", "fgsea_eps", "fgsea_nproc", "fgsea_seed_1W", "fgsea_seed_4W", "target_pathways_plotted", "multiple_testing_scope"),
  value = c(de_1w_file, de_4w_file, "sign(logFC) * -log10(P.Value_for_rank)", "logFC > 0 indicates ACLR > Contra", "P.Value <= 0 or non-finite P.Value replaced with .Machine$double.xmin before rank calculation", length(ranks_1w), length(ranks_4w), sum(rank_1w$p_value_replaced_for_rank), sum(rank_4w$p_value_replaced_for_rank), "Mus musculus", "MSigDB Hallmark collection H", length(pathways), fgsea_min_size, fgsea_max_size, fgsea_eps, fgsea_nproc, fgsea_seed_1w, fgsea_seed_4w, paste(target_pathways, collapse = "; "), "adjusted P values from full Hallmark collection, not only the two target pathways"),
  stringsAsFactors = FALSE
)
write_csv(summary_df, file.path(table_dir, "step13_Figure3B_GSEA_summary.csv"))
write_csv(summary_df[summary_df$metric %in% c("rank_method", "p_value_handling", "fgsea_minSize", "fgsea_maxSize", "fgsea_eps", "fgsea_nproc", "fgsea_seed_1W", "fgsea_seed_4W", "multiple_testing_scope"), ], file.path(table_dir, "step13_fgsea_reproducibility_settings.csv"))
