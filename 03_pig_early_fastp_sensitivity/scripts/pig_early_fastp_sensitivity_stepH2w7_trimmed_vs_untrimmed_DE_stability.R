...
# 这一步是干什么：
# 1) 读取主分析 untrimmed pig early edgeR QLF DE 结果；
# 2) 读取 fastp sensitivity trimmed branch 的 edgeR QLF DE 结果；
...
for (i in seq_len(nrow(contrasts))) {
  ...
  u_pick <- choose_best_de_file(untrimmed_tables_dir, ck, "untrimmed")
  t_pick <- choose_best_de_file(trimmed_tables_dir, ck, "trimmed")

  if (nrow(u_pick$candidates) > 0) all_candidates[[paste0("untrimmed_", ck)]] <- u_pick$candidates
  if (nrow(t_pick$candidates) > 0) all_candidates[[paste0("trimmed_", ck)]] <- t_pick$candidates

  selected <- rbind(
    selected,
    data.frame(
      contrast = cl,
      source_type = "untrimmed",
      selected_file = u_pick$file,
      stringsAsFactors = FALSE
    ),
    data.frame(
      contrast = cl,
      source_type = "trimmed",
      selected_file = t_pick$file,
      stringsAsFactors = FALSE
    )
  )
}
...
if (length(all_candidates) > 0) {
  cand_df <- do.call(rbind, all_candidates)
  row.names(cand_df) <- NULL
  write.csv(cand_df, candidate_file, row.names = FALSE, fileEncoding = "UTF-8")
} else {
  cand_df <- data.frame()
  write.csv(cand_df, candidate_file, row.names = FALSE, fileEncoding = "UTF-8")
}
...
log_msg("Wrote candidate file:", candidate_file)
log_msg("Wrote selected file:", selected_file)

if (any(is.na(selected$selected_file) | selected$selected_file == "")) {
  print(selected)
  stop_with_log(
    "Could not automatically identify all DE tables. Please inspect:", candidate_file,
    "and selected file:", selected_file
  )
}
